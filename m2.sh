...
CURSOR="${SRC}/WhiteSur-cursors/install.sh"
chmod +x "$CURSOR"
bash "$CURSOR"

CURSOR_NAME=""
for d in "$ICONS"/WhiteSur*Cursor* "$ICONS"/whitesur*cursor*; do
    if [[ -d "$d/cursors" ]]; then
        CURSOR_NAME="$(basename "$d")"
        break
    fi
done

if [[ -z "$CURSOR_NAME" ]]; then
    # Current SmallSur historically copies dist/, so support that layout too.
    if [[ -d "${SRC}/WhiteSur-cursors/dist" ]]; then
        while IFS= read -r -d '' d; do
            cp -a "$d" "$ICONS/"
        done < <(find "${SRC}/WhiteSur-cursors/dist" -mindepth 1 -maxdepth 1 -type d -print0)
    fi
    for d in "$ICONS"/WhiteSur*Cursor* "$ICONS"/whitesur*cursor*; do
        if [[ -d "$d/cursors" ]]; then
            CURSOR_NAME="$(basename "$d")"
...
[[ -n "$CURSOR_NAME" ]] || fail "WhiteSur cursor theme was not installed."
[[ -d "${ICONS}/${CURSOR_NAME}/cursors" ]] || fail "Cursor verification failed."
ok "WhiteSur cursors verified: $CURSOR_NAME"
